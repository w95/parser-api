...
async function performRequest(url, params, proxyConfig, startTime) {
    ...
    try {
        ...
        if (network.enabled) {
            ...
            // Add response handler to capture content when show_code is enabled
            if (network.show_code) {
                page.on('response', async (response) => {
                    try {
                        ...
                        // Find corresponding network request
                        const networkRequest = networkRequests.find(req => 
                            req.url === url && req.method === request.method()
                        );
                        
                        if (networkRequest && !networkRequest.blocked && isTextContent(contentType, url)) {
                            try {
                                const responseText = await response.text();
                                networkRequest.content = responseText;
                                networkRequest.content_type = contentType;
                                networkRequest.size = Buffer.byteLength(responseText, 'utf8');
                            } catch (error) {
                                // Ignore errors when reading response body (some responses can't be read)
                                logger.debug(`Could not read response body for ${url}: ${error.message}`);
                            }
                        }
                    } catch (error) {
                        // Ignore errors in response handler to not break the main request
                        logger.debug(`Response handler error: ${error.message}`);
                    }
                });
            }
        }
...
/**
 * Determine if content should be captured based on content type and URL
 */
function isTextContent(contentType, url) {
    ...
    const isTextMime = textMimeTypes.some(type => lowerContentType.includes(type));
    ...
    // Extract file extension from URL (remove query parameters and fragments)
    const cleanUrl = url.split('?')[0].split('#')[0];
    const urlExtension = cleanUrl.substring(cleanUrl.lastIndexOf('.')).toLowerCase();
    const hasTextExtension = textExtensions.includes(urlExtension);
    
    // Binary MIME types to explicitly exclude
    ...
    const isBinaryMime = binaryMimeTypes.some(type => lowerContentType.includes(type));
    
    // Return true if it's text-based and not explicitly binary
    return (isTextMime || hasTextExtension) && !isBinaryMime;
}
